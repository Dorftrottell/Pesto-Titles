...
def ensure_bin(media_pool, bin_name: str):
    """
    Return the bin with *bin_name*, creating it as a local sub-folder of the
    Media Pool root if it does not exist yet.
    Returns the folder object, or raises RuntimeError on failure.
    """
    folder, _ = _find_bin(media_pool, bin_name)
    if folder:
        return folder
    ...
    try:
        ...
        raise RuntimeError(
            f"Bin '{bin_name}' konnte nicht erstellt werden: {e}"
        )
    if not folder:
        raise RuntimeError(
            f"Bin '{bin_name}' konnte nicht erstellt werden "
            "(AddSubFolder lieferte None)."
        )
    return folder
...
def apply_captions(cues: list, template_clip_name: str, track_target: int = 0,
                   progress_cb=None, bin_name: str = "Pesto Captions") -> dict:
    """
    Apply caption cues to timeline using the chosen template clip.

    If the configured bin (*bin_name*) does not exist yet it is created
    automatically.  After the template clip is located anywhere in the
    Media Pool it is copied into that bin so future gallery scans can
    find it there.

    Returns {success, errors, bin_created, clip_placed_in_bin}
    """
    resolve = get_resolve()
    if not resolve:
        raise RuntimeError("Resolve nicht verbunden.")
    ...
    # ── 1. Ensure the Pesto bin exists ────────────────────────────────
    bin_created = False
    _, existing_bin_type = _find_bin(media_pool, bin_name)
    if existing_bin_type is None:
        ensure_bin(media_pool, bin_name)   # raises on failure
        bin_created = True

    # ── 2. Find template clip anywhere in the Media Pool ─────────────
    ...
    if not template_clip:
        raise RuntimeError(
            ...
            "Stelle sicher, dass der Clip irgendwo im Media Pool liegt."
        )

    # ── 3. Copy clip into the Pesto bin if it isn't there already ─────
    clip_placed_in_bin = False
    pesto_bin, _ = _find_bin(media_pool, bin_name)
    if pesto_bin:
        already_there = any(
        ...
        if not already_there:
            try:
                # CopyClips keeps the original in place and adds a
                # reference to the target bin.
                media_pool.MoveClips([template_clip], pesto_bin)
                clip_placed_in_bin = True
            except Exception:
                # MoveClips failed (e.g. PowerBin restriction) — not critical
                pass

    # ── 4. Ensure target track exists ────────────────────────────────
    ...
    return {
        ...
        "clip_placed_in_bin": clip_placed_in_bin,
    }
